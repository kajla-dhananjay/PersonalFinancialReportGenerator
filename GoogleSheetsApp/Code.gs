...
function extractDataFromEmail(emailText) {
  ...
  var creditCardPattern = /Thank you for using your HDFC Bank Credit Card ending 2988 for Rs (\d+\.\d+) at/;
  var upiPattern = /Rs\.(\d+\.\d+) has been debited from account \*\*8368 to VPA ([^\s]+) on (\d{2}-\d{2}-\d{2})\./;
